...
function SideBar({ className }) {
  ...
  return (
    <aside
      className={`${className || "w-full"} flex flex-col gap-6 p-5 lg:p-6`}
    >
      <div className="frosted-panel min-h-[24rem] flex flex-col justify-between rounded-[32px] border border-white/10 p-5 shadow-2xl shadow-slate-950/30">
        <div>
          <div className="mb-8">
            ...
            <p className="mt-2 max-w-sm text-sm text-slate-400">
              A clean messaging experience with a minimal black-and-white theme.
            </p>
          </div>
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/5 p-4 mt-5">
          ...
          <button className="mt-4 w-full rounded-3xl bg-white px-4 py-3 text-sm font-semibold text-black transition hover:bg-slate-100">
            Logout
          </button>
        </div>
      </div>
      ...
      <div className="frosted-panel rounded-[32px] border border-slate-700/70 p-5 shadow-2xl shadow-slate-950/20">
        ...
        <div className="mt-4 space-y-3">
          {friends.map((friend) => (
            <div
              key={friend.id}
              className="flex items-center gap-3 rounded-3xl border border-white/10 bg-[#121212] p-3"
            >
              <img
                src={friend.img}
                alt={friend.name}
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-400">
                  ...
                  <span className="ml-2 capitalize text-slate-400">
                    {friend.status}
                  </span>
                </p>
              </div>
